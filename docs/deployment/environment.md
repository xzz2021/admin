...
模板：`.env.example`。生产或许开发有更新可用 `pnpm env:generate` → 生成 `.env.generated`（`scripts/generate-production-env.mjs`）。
...
| `TOKEN_EXPIRES_TIME`                                | Access 过期时间(秒)                  |
| `TOKEN_REFRESH_EXPIRES_TIME`                        | Refresh 过期时间(秒)                 |
| `SSO_COUNT`                                         | 同用户最大会话数                     |
...
| `PG_DATABASE_URL`                                   | migrate 容器映射为 `PG_DATABASE_URL` |
| `PG_DATABASE_URL`                                   | server 容器映射为 `PG_DATABASE_URL`  |
| `REDIS_HOST` / `PORT` / `PASSWORD`                  | Redis                                |

### 命名注意

Nest Config 与 Prisma 运行时读取的是 **`PG_DATABASE_URL`**。Compose 用 `PG_DATABASE_URL` / `PG_DATABASE_URL` 注入该名。本地 `apps/server/.env` 通常直接写 `PG_DATABASE_URL`。
