...
/** Switch / Rate / ColorPicker：不拉满宽度，也不让左侧 label 通过 for 联动控件 */
const isCompactFormControl = (component?: string) =>
  component === ComponentNameEnum.SWITCH ||
  component === ComponentNameEnum.RATE ||
  component === ComponentNameEnum.COLOR_PICKER
  ...
  setup(props, { slots, expose, emit }) {
    ...
    const hasOptionData = (item: FormSchema) => {
      if (item.component === ComponentNameEnum.TREE_SELECT || item.component === ComponentNameEnum.TRANSFER) {
        return !!item.componentProps?.data?.length
      }
      return !!item.componentProps?.options?.length
    }
    ...
    const renderFormItem = (item: FormSchema) => {
        ...
        default: () => {
          if (item?.formItemProps?.slots?.default) {
            ...
            const Comp = () => {
              ...
              const componentStyle = item.componentProps?.style
                ? item.componentProps.style
                : isCompactFormControl(item.component)
                  ? undefined
                  : { width: '100%' }
      ...
      // Switch 等控件：用无效 for 打断 label 与 input 的关联（空字符串会被 EP 忽略并回退自动关联）
      const disableLabelFor = isCompactFormControl(item.component)
      return (
        <ElFormItem
          v-show={!item.hidden}
